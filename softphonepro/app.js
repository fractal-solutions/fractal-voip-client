...
const defaultSettings = {
  theme: 'dark', accentColor: '#3b82f6', fontSize: 'medium', compact: false,
  audioInput: '', audioOutput: '', ringtone: 'classic', ringVolume: 0.7,
  autoAnswer: false, autoAnswerDelay: 3, echoCancellation: true, noiseSuppression: true,
  dtmfMode: 'rfc2833', camera: '', videoResolution: '720p', selfView: true,
  iceCandidatePolicy: 'all', srtpMode: 'optional', sessionTimers: false,
  debugLog: false, logLevel: 'info'
};

const sampleContacts = [
  { id: generateId(), name: 'Alice Johnson', sipUri: 'sip:alice@example.com', phone: '+15551001', email: '', notes: '', color: '#3b82f6', speedDial: 1 },
  { id: generateId(), name: 'Bob Smith', sipUri: 'sip:bob@example.com', phone: '+15551002', email: '', notes: '', color: '#10b981', speedDial: 2 },
  { id: generateId(), name: 'Charlie Brown', sipUri: 'sip:charlie@example.com', phone: '+15551003', email: '', notes: '', color: '#8b5cf6', speedDial: null },
];
...
// ===== DIALPAD VIEW =====
function DialpadView({ dialString, setDialString, callState, setCallState, callTimer, setCallTimer, addHistory, contacts, addToast, muted, setMuted, onHold, setOnHold }) {
  ...
  const startCall = (type) => {
    ...
    addToast(`Calling ${matchedContact?.name || dialString}...`, 'info');
    setTimeout(() => {
  ...
  const endCall = () => {
    ...
    const dur = callTimer;
    addHistory({
      id: generateId(), direction: 'outgoing', type: 'voice',
...
// ===== SETTINGS VIEW =====
function SettingsView({ profiles, setProfiles, activeProfileId, setActiveProfileId, settings, setSettings, regStatus, setRegStatus, addToast, simulateIncoming }) {
  ...
  const testConnection = () => {
    ...
    setTimeout(() => {
      ...
      addToast('Registration successful (simulated)', 'success');
      setSipLogs(prev => [
        ...prev,
        `[${new Date().toLocaleTimeString()}] REGISTER sip:${profileForm.domain} SIP/2.0`,
        `[${new Date().toLocaleTimeString()}] SIP/2.0 200 OK`,
      ]);
    }, 2000);
  };
        ...
        h('div', null,
              ...
              h('div', { style: { display: 'flex', gap: 6 } },
                p.id !== activeProfileId && h('button', {
                  ...
                  onClick: () => {
                    ...
                    setRegStatus('registering');
                    setTimeout(() => setRegStatus('registered'), 2000);
          ...
          h('div', {
            ...
            h('p', { style: { fontSize: 12, color: 'var(--text-muted)', lineHeight: 1.6, marginBottom: 8 } },
              'This is a simulated softphone UI. To connect to a real FreeSWITCH server, integrate sip.js:'
            ),
            h('pre', {
              className: 'mono',
              style: {
              ...
              `npm install sip.js\n\nimport { UserAgent, Registerer, Inviter } from 'sip.js';\n\nconst ua = new UserAgent({\n  uri: UserAgent.makeURI('sip:1001@your-fs.com'),\n  transportOptions: {\n    server: 'wss://your-fs.com:7443'\n  },\n  authorizationUsername: '1001',\n  authorizationPassword: 'your_pass'\n});\n\nawait ua.start();\nconst registerer = new Registerer(ua);\nawait registerer.register();`
            )
          )
        )
      )),
...
function MainApp() {
  ...
  useEffect(() => {
    if (activeProfile?.registerOnStartup) {
      setRegStatus('registering');
      addToast('Registering with ' + (activeProfile?.domain || 'server') + '...', 'info');
      const t = setTimeout(() => {
        setRegStatus('registered');
  ...
  const simulateIncoming = useCallback(() => {
    ...
    setIncomingCall({ name: names[idx], uri: uris[idx] });
    playRingtone(5000);
  }, []);
    ...
    switch (view) {
      case 'dialpad':
        return h(DialpadView, {
          dialString, setDialString, callState, setCallState,
          callTimer, setCallTimer, addHistory, contacts, addToast,
      ...
      case 'settings':
        return h(SettingsView, {
          ...
          settings, setSettings, regStatus, setRegStatus, addToast, simulateIncoming
        });
      default:
        return h(DialpadView, {
          dialString, setDialString, callState, setCallState,
